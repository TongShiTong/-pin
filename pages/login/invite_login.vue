<template>
    <view class="content-box">
		...
		<view class="welcome">
			...
			<view class="p-login">为了您更好的体验，请先注册</view>
		</view>
        <view class="form">
        	<view>
        		<view class="uni-padding-wrap uni-common-mt">
        			<form @submit="formSubmit" @reset="formReset">
        				<view class="uni-form-item uni-column">
        					...
        					<input class="uni-input" type="number" @input="onInput" name="mobile" v-model="mobile" placeholder="请输入您的手机号" />
        					<!--<button open-type='getPhoneNumber' @getphonenumber='getPhoneNumber'>自动填入</button>-->
        				</view>
						<view v-if="register" class="regis">该手机号已经被注册请直接
						<span @tap='dengluy' style="font-weight: bold;color: #FF5A1F;">登录</span>
						</view>
						<view style="font-size: 22upx; color: #999;text-align: left;">
						    进入猎手聘即代表你同意<span style="color: #666;" @tap="userProtocol">《猎手聘用户协议》</span>
							及<span style="color: #666;" @tap="privacyProtocol">《猎手聘隐私政策》</span>
						</view>
						<!-- 用户协议 -->
						...
						http.apiRequest({
							...
							secuss: (res, httpStatus) => {
								if (res.code == "1") {
									...
									countdown();
									uni.navigateTo({
										url: '/pages/login/login_next/invite_next?mobile=' + mobile + '&tid=' + this.tid,
									});
								} else {
									//短信发送失败
									this.isCountdown = false;
