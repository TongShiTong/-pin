<template>
	<view class="content-box">
		<view class="box">
			<view class="each">
				<!-- <select class="select" @change="chooseTime">
				  <option :value ="time" v-for="(time,index) in timeLists" :key="'1'+index">{{time.value}}</option>
				</select> -->
		...
		<view class="crad-box">
			<view class="crad" v-for="(item,index) in itemsLists" :key="index">
				<view class="name">
					<view>{{item.memo}}</view>
					<view>{{item.money>0?"+"+item.money:item.money}}</view>
				</view>
				<view class="time">
