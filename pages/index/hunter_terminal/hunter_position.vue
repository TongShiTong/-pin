<template>
	<view class="content-box">
		<view class="header">
			<view class="seach">
				<view class="add">杭州</view>
				<view class="frame" @tap="search()">
					<input type="text" placeholder="服务员" v-model="keywordinput" disabled="disabled"/>
					<view><image src="/static/other/seach.png"></image></view>
				</view>
				<view><navigator class="zd" url="/pages/index/hunter_terminal/user/notice/notice" hover-class="none"><view v-if="noticenum > 0" style="background: #FF2727;width: 16upx;height: 16upx;border-radius: 50%;position: absolute;top:-8upx;right: 0;z-index: 1;"></view><image src="/static/user/notice-icon2x.png"></image></navigator></view>
			</view>
		</view>
		<view style="height: 160upx;"></view>
		<view class="ban" @tap="jumpInvite"></view>
		<!-- <view class="banner"></view> -->
		<view class="weijiaona" v-if="!(userInfo.caution_status!=1)">
			<view class="tiyan">
				<view style="display: flex;">
					<image src="/static/other/tishiwj.png"></image>
					<view>未缴纳服务保障金，还可以体验<span style="color: #FF462A;">{{userInfo.laveTask}}</span>单</view>
				</view>
				<navigator class="jiaona" url="/pages/index/hunter_terminal/margin/margin" hover-class="none">去缴纳</navigator>
			</view>
		</view>
		<view v-if="!(userInfo.caution_status!=1)" style="height: 16upx;background: #F7F7F7;"></view>
		<view class="chooes">
			...
			<view class="content" :class="{'fixContente':whether}">
				<view v-if="current === 0">
					<view class="main" v-for="(item,index) in itemsLists" :key="index" @tap="openinfo" :data-itemid="item.jobs_id">
						<view>
						<view class="campany">
							<view class="name-box">
						...
						<!-- <view class="people-num">
							<view class="people-box">
								<view class="people" :class="{'people1':item.remaining==0}">已接单人数</view>
								<view class="num" :class="{'people1':item.remaining==0}">{{item.total-item.remaining}}</view>
							</view>
							<view>
								<view class="people" :class="{'people1':item.remaining==0}">需求人数</view>
								<view class="num" :class="{'people1':item.remaining==0}">{{item.total}}</view>
							</view>
						</view> -->
						</view>
						<view class="info">
				...
				<view v-if="current === 1">
					<view class="main" v-for="(item,index) in itemsLists" :key="index" @tap="qzOpeninfo" :data-itemid="item.id">
						...
						<view style="display: flex;justify-content: space-between;align-items: center;">
							<view class="time-box1" v-if="item.job_tags!=''">
	...
	<!-- 	<navigator url="/pages/index/hunter_terminal/p_detail/p_detail"> 岗位详情</navigator> -->
		...
		<view v-show="linkIsShow">
		  <view class="hb_zzc">
			<view class="link_cons">
				...
				<view style="margin-left: 126upx;font-size: 24upx;color: #fff;opacity: 0.8;margin-top: 166upx;text-align: left;">你当前有</view>
				<view style="margin-left: 126upx;margin-top: 20upx; display: flex;align-items: center;font-size: 32upx;color: #fff;">
					<view style="width: 8upx;height: 30upx;border-radius: 4upx;background: #fff;margin-right: 20upx;"></view>
					<view style="margin-right: 100upx;">待用工</view>
					<view><span style="color: #FC565B;font-size: 36upx;font-weight: bold;padding-right: 10upx;">{{statistics.daiyonggong_JobsTotal}}</span>个</view>
				</view>
				<view style="margin-left: 126upx;margin-top: 20upx; display: flex;align-items: center;font-size: 32upx;color: #fff;">
					<view style="width: 8upx;height: 30upx;border-radius: 4upx;background: #fff;margin-right: 20upx;"></view>
					<view style="margin-right: 100upx;">用工中</view>
					<view><span style="color: #FC565B;font-size: 36upx;font-weight: bold;padding-right: 10upx;">{{statistics.yonggongzhong_JobsTotal}}</span>个</view>
				</view>
				...
				<view @tap="showLinsk"><image style="width: 500upx;height: 192upx;margin-top: 106upx;" src="/static/other/anniu1.png"></image></view>
			</view>
		  </view>
		</view>
		
		<view v-show="hbIsShow">
		  <view class="hb_zzc">
			<view class="link_conss">
				<view class="cloose" @tap="showHongbao"><image style="width: 56upx;height: 56upx;" src="/static/close.png"></image></view>
				<view v-if="isShowhbTop">
					<view class="hb_top"></view>
					<view class="kai_hb" @tap="dakaiHongbao"></view>
				</view>
				<view v-else class="zj_box">
					<view class="zhongjiang">中奖啦</view>
					<view class="jine">{{hongbao}}<span>元</span></view>
					<view class="fyue">钱已放入余额中，<span @tap="tixian">立即提现</span></view>
				</view>
			</view>
		  </view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			tixian() {
				uni.navigateTo({
					url: '/pages/index/hunter_terminal/wallet/wallet'
				});
			},
			// 是否打开类型选择弹框
			openPopup(){
			...
			// 	});
				uni.navigateTo({
					// 跳到搜所页
					url: '/pages/index/hunter_terminal/search/search'
				});
			},
			showLinsk: function () {
				...
				http.apiRequest({
					...
					secuss: (res, httpStatus) => {
						...
						if (res.code == 1) {
							...
							if(this.statistics.daiyonggong_JobsTotal>0 || this.statistics.yonggongzhong_JobsTotal>0) {
								var day1 = new Date();
								day1.setTime(day1.getTime());
								var s2 = (day1.getMonth()+1) + day1.getDate();
								if(s2==uni.getStorageSync("s2")) {
									this.linkIsShow = false
								}else {
									this.linkIsShow = true
									uni.setStorageSync("s2",s2)
								}
							}
						}else {
							uni.showToast({
			...
			dakaiHongbao() {
				http.apiRequest({
					api: "/red_envelope/getRedEnvelope",
					method: "GET",
					data: {
					},
					secuss: (res, httpStatus) => {
						if (res.code == 1) {
							this.isShowhbTop = false
							this.hongbao = res.data.myRedEnvelope.amount
							console.log(res)
						}else {
							uni.showToast({
								title: res.msg
			...
			hongBao() {
				http.apiRequest({
					...
					secuss: (res, httpStatus) => {
						if (res.code == 1) {
							this.hbIsShow=res.data.redEnvelope
							let hbfirst = '0'
		...
		mounted() {
			let that = this
			if(uni.getStorageSync("hbfirst")=="1") {
				this.hongBao()
			}
			var day1 = new Date();
...
<style>
	...
	.header {
		width: 100%;
		height: 160upx;
		background: url(../../../static/index/banner-2.png) no-repeat;
	...
	.ban {
		...
		height: 212upx;
		/* margin-top: 160upx; */
		background: url(../../../static/index/banner-3.png) no-repeat;
		background-size: 100% 100%;
	...
	.link_cons{width: 80%;height: 720upx; border-radius: 40upx;background: #fff;position: relative; background: url(../../../static/other/tankuang1.png) no-repeat;background-size: 600upx 400upx;}
	.link_conss{width: 85%;height: 908upx; border-radius: 40upx;background: #fff;position: relative; background: url(../../../static/index/hongbao-bg.png) no-repeat;background-size: 636upx 908upx;}
	.hb_top {
		width: 100%;height: 360upx;background: url(../../../static/index/hongbao-top.png) no-repeat;background-size: 636upx 360upx;margin: 260upx auto 0;
	}
	.kai_hb {
		width: 398upx;height: 116upx;
		background: url(../../../static/index/hongbao-anniu.png) no-repeat;background-size: 398upx 116upx;margin: 20upx auto 0;
	}
	.zj_box {
		margin-top: 410upx;
		text-align: center;
